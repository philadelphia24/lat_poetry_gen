...
parser.add_argument('--outf', type=str, default='./generated/ten_million/generated_10mio_10k_extra.txt', # CHANGE
...
# Define non-shortening combinations
non_shortening_combinations = ['bl', 'br', 'cl', 'chl', 'cr', 'chr', 'dr', 'fl', 'fr', 'gl', 'gr', 'pl', 'pr', 'tr', 'thr']

# NEW
def is_non_shortening(combo):
    return combo in non_shortening_combinations or combo[0] in ['x', 'z']

# NEW
def should_lengthen(prev_word, current_word):
    ...
    # Handle 'x' and 'z' as special cases
    if current_first_char in 'xz':
        return True
    
    if prev_last_char in 'aeiou' and current_first_char in 'bcdfghjklmnpqrstvwxz':
        if len(current_word) > 1:
            current_second_char = current_word[1]
            if current_second_char in 'bcdfghjklmnpqrstvwxz':
                return not is_non_shortening(current_first_char + current_second_char)
    return False

def lengthen_last_syllable(word_mac):
    syllables = list(word_mac)
    for i in range(len(syllables) - 1, -1, -1):
        if syllables[i] == 's' or syllables[i] == 'u':
            syllables[i] = 'l'
            break
...
with open(args.outf, 'w', encoding='utf-8') as outf:
    with torch.no_grad():
        ...
        for i in range(args.words):
            ...
            word_mac = syllable_dict[word]['mac'] #NEW

            # Check if we need to adjust the last syllable of the previous word
            if should_lengthen(prev_word, word):
                prev_word_mac = lengthen_last_syllable(prev_word_mac)
                
                # Update the last word in the output file
                outf.seek(outf.tell() - len(prev_word_mac) - 1)
                outf.write(prev_word_mac + ' ')
            ...
            outf.write(word_mac + ' ') #before: outf.write(syllable_dict[word]['mac'] + ' ')
